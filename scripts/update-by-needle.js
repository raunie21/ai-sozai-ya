...
if (!needleRaw) {
  console.log('Usage: node scripts/update-by-needle.js <needle>');
  process.exit(1);
}
...
async function main() {
  ...
  for (const h of hits) {
    const newTitle = '観葉植物（室内）';
    const addTags = ['観葉植物','室内','植物','グリーン','インテリア','自然'];
    const mergedTags = Array.from(new Set([...(Array.isArray(h.tags) ? h.tags : []), ...addTags]));
    const updated = { ...h, title: newTitle, tags: mergedTags, category: 'nature', updatedAt: new Date().toISOString() };
    await redis.set(`illustration:${h.id}`, JSON.stringify(updated));
    console.log(`✅ Updated ${h.id} -> title=${newTitle}, tags+=${addTags.join(',')}`);
  }
}
