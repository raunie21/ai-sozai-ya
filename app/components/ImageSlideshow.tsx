...
export default function ImageSlideshow({ illustrations }: ImageSlideshowProps) {
  ...
  // デスクトップ4個、モバイル2個ずつのグループに分割
  const imageGroups = [];
  const groupSize = 4; // デスクトップ用
  for (let i = 0; i < recentImages.length; i += groupSize) {
    imageGroups.push(recentImages.slice(i, i + groupSize));
  }
  
  // モバイル用のグループも作成
  const mobileImageGroups = [];
  const mobileGroupSize = 2; // モバイル用
  for (let i = 0; i < recentImages.length; i += mobileGroupSize) {
    mobileImageGroups.push(recentImages.slice(i, i + mobileGroupSize));
  }
              ...
              <SwiperSlide key={groupIndex}>
                <div className="grid grid-cols-2 gap-3">
                  {group.map((illustration) => (
                    <div
                      key={illustration.id}
                      className="relative bg-white rounded-lg overflow-hidden shadow-md border border-gray-200 transition-all duration-200 hover:shadow-lg cursor-pointer"
                      onClick={() => handleImageClick(illustration)}
                    >
                      <div className="aspect-square relative">
                        {illustration.imageUrl ? (
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-2">
                          ...
                          <p className="text-white/90 text-xs truncate">
                            {illustration.tags?.slice(0, 1).join(' • ')}
                          </p>
                        </div>
                      </div>
                    </div>
