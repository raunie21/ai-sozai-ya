...
export default function IllustrationCard({ illustration, onClick }: IllustrationCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-40 md:h-56 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl md:rounded-2xl flex items-center justify-center text-4xl md:text-6xl mb-3 md:mb-5 relative overflow-hidden border border-gray-200/80 shimmer-effect">
        {/* ダウンロード数表示 - 右上に配置 */}
      ...
      <div className="text-center relative">
        ...
        <div className="text-gray-600 text-xs md:text-sm mb-2 md:mb-4 line-clamp-2">
          {Array.isArray(illustration.tags) ? illustration.tags.slice(0, 2).join(', ') : ''}
        </div>
        
        <button 
          className="button-gradient text-white border-none px-4 md:px-8 py-2 md:py-3 rounded-full cursor-pointer font-semibold text-xs md:text-sm transition-all duration-300 shadow-lg shadow-indigo-500/30 relative overflow-hidden group/btn hover:-translate-y-1 hover:scale-105 hover:shadow-xl hover:shadow-indigo-500/40"
          onClick={(e) => {
            e.stopPropagation();
            onClick();
          }}
